...
class pySELM_Interaction_LAMMPS_PAIR_COEFF(pySELM_Interaction):
	
  def __init__(self, interaction_in=None):
  	  
    if (interaction_in==None): # create a new object (if none specified for wrapper)      
      interaction_in = org.atzberger.application.selm_builder.SELM_Interaction_LAMMPS_PAIR_COEFF();
      
    super(pySELM_Interaction_LAMMPS_PAIR_COEFF, self).__init__(interaction_in);

  # getters
  def getTableFilename(self):
    return self.wrappedObj.getTableFilename();
    
  def getEnergyEntryName(self):
    return self.wrappedObj.getEnergyEntryName();
  ...
  def getCoefficient(self):
    return self.wrappedObj.getCoefficient();
  ...
  # setters
  def setTableFilename(self, val_in):
    self.wrappedObj.setTableFilename(val_in);
      
  def setEnergyEntryName(self, val_in):
    self.wrappedObj.setEnergyEntryName(val_in);
  ...
  def setCoefficient(self, val_in):
    self.wrappedObj.setCoefficient(val_in);    
           
  def setPairData(self, numPairs_in, pairList_lagrangianI1_in, pairList_typeI1_in, pairList_lagrangianI2_in, pairList_typeI2_in, coefficient_in):
    self.wrappedObj.setPairData(numPairs_in, createJythonUnwrappedList(pairList_lagrangianI1_in), pairList_typeI1_in, createJythonUnwrappedList(pairList_lagrangianI2_in), pairList_typeI2_in, coefficient_in);    
  ...
  # misc.   
  def addTypePair(self, lagrangianI1_in, typeI1_in, lagrangianI2_in, typeI2_in, coefficient_in):
    self.wrappedObj.addTypePair(lagrangianI1_in.getLagrangianObj(), typeI1_in, lagrangianI2_in.getLagrangianObj(), typeI2_in, coefficient_in);
    
  def removeTypePair(self, lagrangianI1_in, typeI1_in, lagrangianI2_in, typeI2_in):
    self.wrappedObj.removeTypePair(lagrangianI1_in.getLagrangianObj(), typeI1_in, lagrangianI2_in.getLagrangianObj(), typeI2_in);
