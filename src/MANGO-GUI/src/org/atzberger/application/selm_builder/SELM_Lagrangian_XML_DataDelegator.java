 ...
 */
public class SELM_Lagrangian_XML_DataDelegator implements Atz_XML_SAX_DataHandlerInterface {

  String     xmlString         = "";
  Attributes xmlAttributes = null;

  SELM_Lagrangian lagrangianDataHandler = null;

  String lagrangianName    = "";
  String lagrangianTypeStr = "";
  ...
  public void XML_startElement(String uri, String localName, String qName, Attributes attributes, Atz_XML_SAX_DataHandler sourceHandler) throws SAXException {
    ...
    if (!flagDelegateParsing) {
      ...
      if (qName.equals(SELM_Lagrangian.tagXML_SELM_Lagrangian)) { /* detect SELM_Lagrangian tag */
        flagDelegateParsing = false; /* initialize when encountering tag */
      }

      if (qName.equals(SELM_Lagrangian.tagXML_LagrangianName)) {
        //sourceHandler.parseNextTagWithDataHandler(this); /* not technically required */
      }

      if (qName.equals(SELM_Lagrangian.tagXML_LagrangianTypeStr)) {
        //sourceHandler.parseNextTagWithDataHandler(this); /* not technically required */
      }

    } else { /* parse using by delegating to the specific handler */
      lagrangianDataHandler.XML_startElement(uri, localName, qName, attributes, sourceHandler);
    }
  ...
  public void XML_characters(char[] ch, int start, int length, Atz_XML_SAX_DataHandler sourceHandler) throws SAXException {
    ...
    if (!flagDelegateParsing) {
      ...
      lagrangianDataHandler.XML_characters(ch, start, length, sourceHandler);
    }
  ...
  public void XML_endElement(String uri, String localName, String qName, Atz_XML_SAX_DataHandler sourceHandler) throws SAXException {
    ...
    if (!flagDelegateParsing) { /* not delegating parsing */
      ...
      if (qName.equals(SELM_Lagrangian.tagXML_LagrangianName)) {
        lagrangianName = (String) xmlAttributes.getValue("value");
      }

      if (qName.equals(SELM_Lagrangian.tagXML_LagrangianTypeStr)) {

        lagrangianTypeStr = (String) xmlAttributes.getValue("value");

        /* setup the parser to use for the remaining data */

        /* use Reflection to instantiate class for this data type */

        String packageName         = SELM_Lagrangian.class.getPackage().getName();//"pja_desktopapplication1";
        String baseName            = SELM_Lagrangian.class.getSimpleName();
        String lagrangianClassName = packageName + "." + baseName + "_" + lagrangianTypeStr;

        lagrangianDataHandler = (SELM_Lagrangian) loadAndInstantiateClass(lagrangianClassName);
        lagrangianDataHandler.LagrangianName = lagrangianName;
        
        flagDelegateParsing = true; /* delegate parsing to hander */
      }

    } else { /* parse using by delegating to the specific handler */
      lagrangianDataHandler.XML_endElement(uri, localName, qName, sourceHandler);
    }

    if (qName.equals(SELM_Lagrangian.class.getSimpleName())) { /* SELM_Lagrangian tag */
  ...
  public Object XML_getData() {
    return lagrangianDataHandler.XML_getData();
